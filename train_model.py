...
for i in range(10):
    mock_f1 = round(random.uniform(0.7, 1.0), 3)
    mock_fpr = round(random.uniform(0.0, 0.3), 3)
    mock_fnr = round(random.uniform(0.0, 0.3), 3)
    
    logging.info(f"Mock run {i+1}: F1 Score: {mock_f1}")
    logging.info(f"Mock run {i+1}: False Positive Rate: {mock_fpr}")
    logging.info(f"Mock run {i+1}: False Negative Rate: {mock_fnr}")
    time.sleep(2)